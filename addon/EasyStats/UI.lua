...
function ES:CreateUI()
    ...
    frame:SetSize(286, 250); frame:SetMovable(true); frame:EnableMouse(true); frame:SetClampedToScreen(true)
    ...
    local title = frame:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge"); title:SetPoint("LEFT", icon, "RIGHT", 8, 0); title:SetWidth(170); title:SetJustifyH("LEFT")
    ...
    local stats = frame:CreateFontString(nil, "OVERLAY", "GameFontHighlight"); stats:SetPoint("TOPLEFT", 14, -70); stats:SetWidth(258); stats:SetJustifyH("LEFT")
    ...
    local rows = {}
    for index = 1, 4 do
        local row = CreateFrame("Button", nil, frame); row:SetSize(258, 25); row:SetPoint("TOPLEFT", 14, -110 - (index - 1) * 25)
        local rank = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall"); rank:SetPoint("LEFT"); rank:SetText(index .. ".")
        local itemIcon = row:CreateTexture(nil, "ARTWORK"); itemIcon:SetSize(20, 20); itemIcon:SetPoint("LEFT", 20, 0)
        local name = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall"); name:SetPoint("LEFT", itemIcon, "RIGHT", 6, 0); name:SetWidth(205); name:SetJustifyH("LEFT")
        row.icon, row.name = itemIcon, name
        row:SetScript("OnEnter", function(r)
            if r.itemID then GameTooltip:SetOwner(r, "ANCHOR_RIGHT"); GameTooltip:SetItemByID(r.itemID)
    ...
    local talentLabel = sectionLabel(frame, self.L.TALENTS, -215); talentLabel:EnableMouse(true)
    ...
    local support = frame:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall"); support:SetPoint("TOPLEFT", 14, -231)
    ...
    function ui:SetUnavailable(message)
        self.title:SetText(ES.L.ADDON_NAME); self.stats:SetText(message); self.apply:Disable()
        for _, row in ipairs(self.rows) do row.name:SetText("—"); row.icon:SetTexture(nil); row.itemID = nil end
    end
    function ui:Render(spec, data)
        ...
        if not data then self:SetUnavailable(ES.L.NO_DATA); return end
        ...
        for index, row in ipairs(self.rows) do
            ...
            if item then ES:LoadItem(item.itemID, function(name, texture, link)
                if row.itemID == item.itemID then row.name:SetText(name); row.icon:SetTexture(texture); row.link = link end
            end) end
        end

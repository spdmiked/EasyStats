...
EasyStats has two security boundaries. The Python 3.12 pipeline is the only component that can use the network. It discovers specialization-ranked Mythic+ characters through Raider.IO's public leaderboard data, enriches them through APIs, validates observations, removes character identity, aggregates each specialization, and atomically emits `GeneratedData.lua`. The Lua addon only reads those aggregates.
